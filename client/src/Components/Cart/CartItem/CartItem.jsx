...
function CartItem({ name, quantity, price, image, id, changeVariable }) {
  ...
  let { userInfo, backendCart } = useSelector((state) => state);
  const deleteProduct = (id) => {
    dispatch(deleteP(id));
    dispatch(updateCartQuantity());
    if (userInfo.id && backendCart[0]?.id) {
      dispatch(deleteFromCartBack(backendCart[0].id, id));
      setTimeout(() => {
  ...
  const changeModify = async (e) => {
    if (e.target.value === '-') {
      ...
      if (userInfo.id && backendCart[0]?.id) {
        ...
        await dispatch(putCartFromBack(backendCart[0].id, id, quantity));
